...
$(document).ready(function() {
      ...
      <div class="box_15">
      <?php
	  $st=mysql_query("SELECT * FROM config");
	  $sta=mysql_fetch_assoc($st);
	  ?>
      <?php if($sta['aberto']=='1'){ ?>
...
<div id="novas"></div>
    <?php
    $pedido=mysql_query("SELECT * FROM store_finalizado WHERE status='1' GROUP BY id_pedido order by id desc");
...
<div class="box_193" id="p2" style="display:none;">
    <?php
    $pedido=mysql_query("SELECT * FROM store_finalizado WHERE status='4' GROUP BY id_pedido order by id desc");
    while($pedidos=mysql_fetch_assoc($pedido)){
    $cli=mysql_query("SELECT * FROM usuarios WHERE id_u='".$pedidos['id_estrangeiro']."'");
    $cliente=mysql_fetch_assoc($cli);
...
<div class="box_193" id="p3" style="display:none;">
    <?php
    $pedido=mysql_query("SELECT * FROM store_finalizado WHERE status='5' GROUP BY id_pedido order by id desc");
    while($pedidos=mysql_fetch_assoc($pedido)){
    $cli=mysql_query("SELECT * FROM usuarios WHERE id_u='".$pedidos['id_estrangeiro']."'");
    $cliente=mysql_fetch_assoc($cli);
