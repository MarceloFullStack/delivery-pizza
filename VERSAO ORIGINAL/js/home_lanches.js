...
$("#cadastrar").click(function() {
	...
	$('#pagamento_dinheiro').removeClass("ativo").addClass("inativo");
			...
			$('body').on('click', '.add-lanche', function(){
				...
				$('#prato').attr("data-escolher", escolha);
				$('#prato').attr("data-foto", foto);
				$('#prato').attr("data-maximo", maximo);
				...
				if(escolha=='2'){
					...
					$('#boxingredientes').append('<li class="li-'+TableOfContentsSplit[number].replace(/\s/gi, '')+'"><i class="quantidade_ingredientes" id="qtd-'+TableOfContentsSplit[number].replace(/\s/gi, '')+'"> 1</i>'+TableOfContentsSplit[number]+' <div class="carrinho_lanche"><a id="'+TableOfContentsSplit[number].replace(/\s/gi, '')+'" class="maisum_i" data-nome="'+TableOfContentsSplit[number].trim()+'"></a><input id="inpu-i-'+TableOfContentsSplit[number].replace(/\s/gi, '')+'" type="text" class="in-qtdd-item-ped" readonly="on" value="1"><a id="'+TableOfContentsSplit[number].replace(/\s/gi, '')+'" data-nome="'+TableOfContentsSplit[number].trim()+'" class="menosum_i"></a></div></li>');
			...
			$(document).on("click",".maisum_i",function(){
			   var id          =   $(this).attr('id');
			   var valor       =   $('#valor_pizza').text();	
			   var quantidade  =   $('#inpu-i-'+id).val();
			   var nome        =   $(this).attr('data-nome');
			   var produto     =   $('#prato').attr("data-ident");
			   var quant       =   parseFloat(quantidade) + 1;



			   $.post('/add_opcionais_lanche.php', {nome: nome, produto: produto, quantidade: quant}, function(resposta) {
			   if(resposta == 1){
			      if(quantidade>0){
			      $.post('/retornar_valor_ingrediente.php', {nome: nome}, function(resposta) {
			      var total = (parseFloat(resposta) + parseFloat(valor)).toFixed(2);
			      $('#valor_pizza').text(total);
			      }); 
			   
			      var quantidade2  = parseFloat(quantidade) + 1;
			      $('#inpu-i-'+id).val(quantidade2);
			      $('#qtd-'+id).text(quantidade2);
			      }
			    }else{
				$.alert({
			title: 'Limite de ingredientes',
			content: 'Quantidade maxima excedida',
			confirmButton: 'Ok',
			...
			$(document).on("click",".menosum_i",function(e){
			   var id = $(this).attr('id');
			   var quantidade  = $('#inpu-i-'+id).val();
			   var valor       =   $('#valor_pizza').text();
			   var nome        =   $(this).attr('data-nome');
			   ...
			   $.post('/add_opcionais_lanche.php', {nome: nome, produto: produto, quantidade: quant});
			   ...
			   $.post('/retornar_valor_ingrediente.php', {nome: nome}, function(resposta) {
			   var total = (parseFloat(valor) - parseFloat(resposta)).toFixed(2);
			   $('#valor_pizza').text(total);
			   });
...
$('body').on('click', '#maislanche', function(){
		...
		var escolher        = $('#prato').attr("data-escolher");
		var nome            = $('#prato').attr("data-nome");
		var maximo          = $('#prato').attr("data-maximo");
		...
		$('.quantidade_ingredientes').each(function() { total_ingred += +$(this).text();});
		if(total_ingred > maximo){
		$.alert({
			title: 'Limite excedido',
			content: 'Por favor escolha no maximo '+maximo+' itens.',
			confirmButton: 'Ok',
			confirmButtonClass: 'btn-info',
			animation: 'bottom',
			icon: 'fa fa-check',
		...
		if(escolher=='2'){
        var ingredientes    = $('#boxingredientes').map(function() { return $(this).text() }).get().join(",");
		}
		var valor           = $('#valor_pizza').text();
