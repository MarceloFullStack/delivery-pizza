...
<?php
if($_POST[buscar]) {
echo '<script>location.href="'.raiz.'adm/adm-pedidos-hoje/1/'.post(buscarid).'";</script>';	
}
...
if(get(id3) == deletar) {
    ...
    echo '<meta http-equiv="refresh" content=0;url="'.raiz.'adm/adm-pedidos-hoje/'.get(id2).'/">';
} else {

if($_GET[id3]) {
$busca = "select * from ".$cfg_sv[prefixo]."pedidos where id='".get(id3)."' and data='$dt' order by id desc";	
$busca2 = $mysqli -> query ("select * from ".$cfg_sv[prefixo]."pedidos where id='".get(id3)."' and data='$dt' order by id desc");	
} else {
$busca = "select * from ".$cfg_sv[prefixo]."pedidos where data='$dt' order by id desc";
$busca2 = $mysqli -> query ("select * from ".$cfg_sv[prefixo]."pedidos where data='$dt' order by id desc");
}
...
<td><a href='".raiz."adm/adm-pedidos-hoje/".get(id2)."/deletar/$exibir[id]'><button type='button' rel='tooltip' title='Deletar' class='btn btn-danger'> Deletar pedido</button></a></td>
...
echo '<script>location.href="'.raiz.'adm/adm-pedidos-hoje/'.get(id2).'/#'.post(id).'";</script>';
}
...
if ($pc<$tp) {
echo "<a href='".raiz."adm/adm-pedidos-hoje/$proximo/'> <input type='submit' value='Próxima ($proximo)' style='margin:1%;' class='btn btn-info btn-fill pull-right'></a>";
}

echo "Página ".get(id2)."";

if ($pc>1) {
echo " <a href='".raiz."adm/adm-pedidos-hoje/$anterior/'> <input type='submit' value='Anterior ($anterior)' style='margin:1%;' class='btn btn-info btn-fill pull-right'></a>";
}
?> 
